-- Nightingale Connect Database Schema - Final Fixed Version
-- This script fixes the RLS recursion issue properly
...
DROP FUNCTION IF EXISTS is_room_admin(uuid);
...
DROP POLICY IF EXISTS "members_delete_self" ON members;
...
-- Helper: is the current user an admin of a room?
CREATE OR REPLACE FUNCTION is_room_admin(_room uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE room_id = _room AND user_id = auth.uid() AND role = 'admin'
  );
$$;

-- Helper: is the current user a member of a room?
CREATE OR REPLACE FUNCTION is_room_member(_room uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
...
ALTER FUNCTION is_room_admin(uuid) OWNER TO postgres;
ALTER FUNCTION is_room_member(uuid) OWNER TO postgres;

-- =============================================
-- STEP 6: CREATE NON-RECURSIVE RLS POLICIES
-- =============================================

-- Messages policies (can reference members safely)
CREATE POLICY "messages_select_policy" ON messages
  FOR SELECT USING (is_room_member(room_id));

CREATE POLICY "messages_insert_policy" ON messages
  FOR INSERT WITH CHECK (
    sender_id = auth.uid() AND is_room_member(room_id)
  );
...
-- Rooms policies (can reference members safely)
...
-- Members policies (NO self-referencing queries)
...
CREATE POLICY "members_delete_self" ON members
  FOR DELETE USING (user_id = auth.uid());

-- Admin policies for managing other members
...
AND routine_name IN ('is_room_admin', 'is_room_member')
ORDER BY routine_name;

-- Success message
SELECT 'Schema fixed successfully - no recursion, proper security!' as status;
